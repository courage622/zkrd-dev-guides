...
- [1. 流程概览](#1-流程概览)
  - [1.1 目的](#11-目的)
  - [1.2 首次交付流程](#12-首次交付流程)
  - [1.3 持续迭代流程](#13-持续迭代流程)
  - [1.4 分支与 Tag 职责](#14-分支与-tag-职责)
- [2. 日常开发与协作](#2-日常开发与协作)
...
首次交付适用于从空白 GitLab 项目开始，完成代码库初始化、首轮功能开发并发布第一个正式版本的场景。

首次交付期间，`main` 是项目初始化基线；首个版本发布成功后，`main` 开始作为生产稳定主线。首个版本可根据项目成熟度选择：

| 项目状态 | 建议首个版本 |
|---|---|
| 内部试用、功能仍可能明显变化 | `0.1.0` |
| 已达到稳定交付标准并承诺兼容性 | `1.0.0` |

```mermaid
flowchart TB
    A["创建 GitLab 项目"] --> B["初始化 main 和基础文件"]
    B --> C["从 main 创建 dev"]
    C --> D["配置成员、保护规则和 CI"]
    D --> E["确定首发范围和验收标准"]
    E --> F["feature/fix 分支开发"]
    F --> G["MR → dev<br/>CI + 代码审查"]
    G --> H{"首发范围完成？"}
    H -->|否| F
    H -->|是| I["创建 release/vX.Y.Z"]
    I --> J["版本准备、完整回归和审批"]
    J --> K["MR → main"]
    K --> L["创建首个 Tag 和 Release"]
    L --> M["构建部署并验证"]
    M --> N["发布修改回流 dev"]
    N --> O["进入持续迭代"]
```

初始化阶段由 Maintainer 完成：
...
基础文件至少应包含 `README.md`、`VERSION`、`CHANGELOG.md`、`.gitignore` 以及项目实际需要的源码、测试、依赖和 CI 配置。创建 `dev` 后，应按照 [GitLab 项目配置与开发管理规范](./GITLAB_PROJECT_MANAGEMENT.md) 保护 `main`、`dev` 和 `v*` Tag。

首轮功能使用本规范第 2 章的任务分支、MR、CI 和代码审查流程开发。创建发布分支前，应确认：

- 首发范围内的任务均已合入 `dev`。
- 主要功能、异常流程、安全和权限控制已经测试。
- README、配置模板、数据库 migration、部署和回滚说明完整。
- CI 持续通过，`CHANGELOG.md` 已记录首发的重要变化。

达到首发条件后，按照第 3 章创建 `release/vX.Y.Z`，完成版本准备、回归测试、合入 `main`、创建首个 Tag、构建部署和修改回流。确认 `main`、`dev` 均包含发布修改后，删除发布分支。
...
flowchart TB
    ...
    R3 --> BACKDEV["发布修复回流 dev<br/>保证 dev 不丢修复"]
